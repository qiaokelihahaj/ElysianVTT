...
### 4. 交互与选中态系统 (Selection State)
实体是可点选的。当用户点击画布上的实体时，该实体被设为 `selectedEntityId`，并在渲染层高亮显示（黄色边框）。
点击地图空白处会清除选中（进入未选中态）；但在"选择移动目标"模式下，空白点击改为设置移动目的地。
HUD 的血条与所有操作按钮（施法、移动、交互）都基于当前选中目标，使整个交互流成为一个一致的"选中→操作"模型。
这个设计为 GM 端提供了最大的叙述自由——可以直接控制任何实体（道具、敌人、环境物体），未来会通过权限系统进一步细化（玩家只能控制自己的角色）。
